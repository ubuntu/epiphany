...
*[![epiphany](https://snapcraft.io/epiphany/badge.svg)](https://snapcraft.io/epiphany)
[![epiphany](https://snapcraft.io/epiphany/trending.svg?name=0)](https://snapcraft.io/epiphany)*
...
git remote add ubuntu https://github.com/ubuntu/epiphany.git
git fetch --all
```
...
5. Once the PR has been merged into the stable branch, then on the next git mirror sync (every 4 hours), launchpad will trigger [a build of the snap that gets published](https://launchpad.net/~desktop-snappers/+snap/epiphany) to the [snap store](https://snapcraft.io/epiphany) into the *candidate* channel. After sufficient testing of the snap from the candidate channel, then the reviewer (a Collaborator of the snap in the store) will promote the snap to the stable branch in the snap store.
